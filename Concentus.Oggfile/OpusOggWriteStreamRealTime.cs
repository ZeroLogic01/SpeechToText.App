...
{
    /// <summary>
    /// A class for writing real-time audio data as an .opus Ogg stream, using an Opus encoder provided in the constructor.
    ...
    /// </summary>
    public class OpusOggWriteStreamRealTime
    {
        ...
        private Stream _opusHeaderStream;
        private Crc _crc;
        ...
        public Stream EncodedAudioStream { set; get; }
        ...
        /// <param name="opusHeaderStream">A base stream to accept the encoded ogg file output</param>
        ...
        /// the encoding).</param>
        public OpusOggWriteStreamRealTime(OpusEncoder encoder, Stream opusHeaderStream, OpusTags fileTags = null, int inputSampleRate = 0)
        {
            _encoder = encoder;
            ...
            _opusHeaderStream = opusHeaderStream;
            _opusFrameIndex = 0;
            _granulePosition = 0;
        ...
        /// When a real-time low latency streaming is needed, set it to true</param>
        public void WriteSamples(short[] data, int offset, int count)
        {
            ...
            {
                throw new NullReferenceException("EncodedAudioStream cannot be null!");
            }
        ...
        public void Finish()
        {
            ...
            _opusHeaderStream.Flush();
            _opusHeaderStream.Dispose();
            EncodedAudioStream?.Flush();
            //  EncodedAudioStream?.Dispose();
            _finalized = true;
        }
        ...
        /// </summary>
        /// <param name="isNewPageRequied">A boolean value indicating whether creation of a new page is required or not</param>
        /// <param name="isEncodedAudioStreamWrite">A boolean value to determine which stream to write</param>
        private void FinalizePage(bool isNewPageRequied = true, bool isEncodedAudioStreamWrite = false)
        {
            if (_finalized)
            {
            ...
            if (_lacingTableCount != 0)
            {
                ...
                if (isEncodedAudioStreamWrite)
                {
                    try
                    {
                        EncodedAudioStream?.Write(_currentHeader, 0, _headerIndex);
                        EncodedAudioStream?.Write(_currentPayload, 0, _payloadIndex);
                    }/* since we are passing EncodedAudioStream from within a using statement.
                    This stream will be closed when Finish() method gets called, so no need to raise
                    exception*/
                    catch (ObjectDisposedException) { }
                }
                else
                {
                    _opusHeaderStream.Write(_currentHeader, 0, _headerIndex);
                    _opusHeaderStream.Write(_currentPayload, 0, _payloadIndex);
                }

                // reset the page if required
                if (isNewPageRequied)
                {
                    BeginNewPage();
                }
